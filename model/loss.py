...
#                     print(gb_correlate.requires_grad)
                ...
                if visualize:
                    ...
                    def normalize(arr):
                        ...
                        arr /= np.max(arr)
                        return np.moveaxis(arr,0,-1)
                    hmps.append(reshapeNormalize(ndimage.zoom(firstCompare.cpu().detach().numpy(),(1,16,16),mode='nearest',order=0)))
                    gbimgs.append(reshapeNormalize(ndimage.zoom(secondCompare.cpu().detach().numpy(),(1,16,16),mode='nearest',order=0)))
